...
def project_function(periodReturns, periodFactRet, x0=[]):
    ...
    if T == 60: # 12 months * 5 years 
        ...
        try:
            ...
            # print(c)
            x = Strategy.execute_strategy(48, periodReturns, c=10, p=p)
...
def find_params(Strategy, all_params, periodReturns, T):
    """Iterates through all_params to determine the set of parameters that result in the optimal 
    Sharpe ratio during the calibration period

    Args:
        Strategy (class): risk parity portfolio optimization model
        all_params (np.ndarray): all possible parameters
